...
const applySubexpressionDefaults = expr => {
  ...
  out.ignoreFlags = ('ignoreFlags' in out) ? out.ignoreFlags : false;
  out.ignoreStartAndEnd = ('ignoreStartAndEnd' in out) ? out.ignoreStartAndEnd : false;
...
const t = {
  ...
  subexpression: asType('subexpression', { containsChildren: true }),
...
class SuperExpressive {
  ...
  static [evaluate](el) {
    switch (el.type) {
      ...
      case 'subexpression': {
        const inner = el.value.map(SuperExpressive[evaluate]).join('');
        return `(?:${inner})`;
      }
